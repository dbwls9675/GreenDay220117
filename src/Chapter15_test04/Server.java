...
public class Server extends JFrame {
		...
		start.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (textField.getText().equals("") || textField.getText().length() == 0) {
					JOptionPane.showMessageDialog(null, "포트 번호를 입력하세요", "경고 메시지", JOptionPane.ERROR_MESSAGE);
	...
	class ThreadHandler extends Thread {
		...
		// =================================================
		/*
		 * Socket 데이터 통신 => 클라이언트가 연결 요청을 하고 서버가 연결 수락을 했다면, 양쪽의 Socket 객체로부터 각각
		 * 입력스트림(InputStream)과 출력스트림(OutputStream)을 얻을 수 있다.
		 */
		public void user_network() {
			try {
				/*
				 * 보조 스트림은 자체적으로 입출력을 수행할 수 없기 때문에 InputStream, OutputStream 등에 연결해서 입출력을 수행한다.
				 */
